...
export default function BooksPage() {
  ...
  useEffect(() => {
    const fetchBooks = async () => {
      try {
        ...
        const uniqueGenres = Array.from(new Set(data.flatMap((book) => book.genre)))
  ...
  const handleToggleFavorite = async (bookId: string) => {
    ...
    try {
      ...
      // Update local state
      const updatedBooks = books.map((b) => (b.id === bookId ? { ...b, isFavorite: !b.isFavorite } : b))
      setBooks(updatedBooks)

      // Update filtered books as well
      setFilteredBooks((prevFiltered) =>
        prevFiltered.map((b) => (b.id === bookId ? { ...b, isFavorite: !b.isFavorite } : b)),
      )
    } catch (error) {
  ...
  return (
    <div className="md:ml-64">
      ...
      {filteredBooks.length === 0 ? (
        ...
        <BookGrid books={filteredBooks} onToggleFavorite={handleToggleFavorite} />
      )}
    </div>
  )
}
